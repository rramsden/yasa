...
stream(Data, Req, State = #state{registered = Registered}) ->
    ...
    case Type of
        ...
        <<"register">> ->
            {200, Values} = yasa_web_api:reply(Key, <<"get">>, Json),
            Reply = [{init, true}, {key, Key}, {values, Values}],
            {reply, jsx:to_json([Reply]), Req, State#state{registered = [Key | Registered]}};
        <<"gauge">> ->
            yasa_web_api:reply(Key, <<"gauge">>, Json),
            {reply, <<"ok">>, Req, State};
        <<"counter">> ->
            yasa_web_api:reply(Key, <<"counter">>, Json),
            {reply, <<"ok">>, Req, State};
        Type ->
...
info({timeout, _TRef, tick}, Req, State = #state{registered = Registered}) ->
    erlang:start_timer(?TICKER, self(), tick),
    Mapper = fun(Key) ->
            {200, Values} = yasa_web_api:reply(Key, <<"get">>, [{<<"range">>, <<"-10sec">>}]),
